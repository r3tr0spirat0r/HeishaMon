...
// this will be set to true if, during boot, heishamon detects this heatpump has extra data block (like K and L series do)
...
unsigned long sendCommandReadTime = 0;

//set to millis value during send, allow to wait millis for answer
...
//set neopixel to yellow to indicate lost wifi
#endif
    }
...
#else
  if (WiFi.localIP()) {      //WiFi connected and IP>0
    neoPixelState = pixels.Color(0, 0, 0);
    if (((WiFi.getMode() & WIFI_MODE_AP) != 0) && ((heishamonSettings.wifi_ssid[0] != '\0') || (!heishamonSettings.hotspot)) ) {
      log_message(_F("WiFi (re)connected, shutting down hotspot..."));
      WiFi.softAP("");
      WiFi.softAPdisconnect(true);
    }
...
//resend all 1wire values to mqtt
        resetlastalldatatime();

//resend all heatpump values to mqtt
      }
...
//subscribe to raw heatpump data over MQTT
      }
#endif
    }
  }
...
//+32 long enough to contain millis() and the json part later for websocket mesg
...
//read available data and place it after the last received data
    proxylen++;
...
//return so this while loop does not loop forever if there happens to be a continous invalid data stream
    }
  }
  //if ((proxylen > 0) && (proxydata_length == 0 )) proxy_totalreads++;

//this is the start of a new read
...
if (heishamonSettings.logHexdump) logHex(proxydata, proxydata_length);
    ...
    if (proxydata_length == (proxydata[1] + 3)) { //we received all data (serial2_data[1] is header length field)
      sprintf_P(log_msg, PSTR("PROXY Received %i bytes"), proxydata_length);
...
//should contain valid checksum also
          }
...
//should containt valid checksum also
          }
        } else {
...
//set the flag to true so we know we can request this data always
...
//then calculcated checksum byte afterwards
...
//simple semaphore to make sure we don't have two callbacks at the same time
    char msg[length + 1];
...
//strip base plus seperator from topic
...
//strip the first 17 "s0/WatthourTotal/" from the topic to get the s0 port
...
//strip the first 9 "commands/" from the topic to get what we need
...
//strip the opentherm subtopic from the topic
...
//strip the gpio subtopic from the topic
...
if (!g_ethStarted) {
  ...
  if (ip == IPAddress(0,0,0,0)) ethernetStat = F("connected - no IP");
  else ethernetStat = String(F("connected - IP ")) + ip.toString();
}
#else
